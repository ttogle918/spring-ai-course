...
- 단일 책임: 에이전트는 좁게 설계하고 가능하면 DTO를 반환하세요.
- 도구 의미론: `@Tool` 메서드는 조율자와 에이전트 간 브리지입니다. 멱등성과 부작용 안전성에 유의하세요.
- 스레딩: 조율자는 워커 스레드를 생성할 수 있습니다. `SseEmitter`를 전달할 때 `InheritableThreadLocal`을 신중히 사용하고 공유 가변 상태를 피하세요.

예제: 요청 흐름

1. 사용자가 자유 텍스트 요청을 전송합니다.
2. `TravelOrchestrator.parseUserQuery()`가 LLM을 호출해 구조화된 `Requirements`를 추출합니다.
3. 조율자는 에이전트 툴(관광지/맛집/숙소)을 병렬로 호출해 도메인 DTO를 수집합니다.
4. `PlanAgent`는 DTO들을 합쳐 LLM에게 최종 `Plan` 엔티티 생성을 요청합니다.
5. `BudgetAgent`가 비용을 검증하고 필요 시 재계획을 트리거합니다.

추가 팁

- 프롬프트에 명시적 JSON 예시를 포함하고 잘못된 출력에 대비한 수리 프롬프트를 추가하세요.
- 에이전트별 지연시간과 토큰 사용량을 계측해 비용이 큰 단계를 식별하세요.
- 프로덕션 환경에서는 LLM 공급자 어댑터를 분리하고 재시도/백오프 로직을 중앙화하세요.
